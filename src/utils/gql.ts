...
export async function getNameAndGatewayContracts(
  ...
  const gql = new Graphql(
    grid.getDefaultUrls(window.config.network as NetworkEnv).graphql,
  );
  ...
  const whereNameContract = `
  (where: { 
    twinID_eq: ${twinId}, name_eq: "${window.config.projectNamePrefix}${twinId}${name}",
    state_in: [Created, GracePeriod] }, orderBy: twinID_ASC
  `;
  const nameContractsCount = await gql.getItemTotalCount(
    'nameContracts',
    `${whereNameContract})`,
  );
  const nodeContractsCount = await gql.getItemTotalCount(
    'nodeContracts',
    `${whereNodeContract})`,
  );
