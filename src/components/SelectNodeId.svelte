...
<script lang="ts">
  ...
  import type { NodeInfo } from '@threefold/grid_client/dist/node';
  ...
  const loadNodes = debounce(
    async (
      ...
      nodes = await findNodes(deployer.value.mnemonic, filters);
  ...
  onMount(async () => {
    ...
    f = new FormGroup({
      cru: deployer.get('cpu'),
      mru: deployer.get('memory'),
      sru: deployer.get('disk'),
      publicIPs: deployer.get('ipv4'),
      region: deployer.get('region'),
      certified: deployer.get('certified'),
      farmId,
    });
    unsubscribe = f.subscribe(() => {
      if (farmLoaded) {
        loading = true;
        deployer.get('nodeId').setValue(null);
        loadNodes(f);
      }
    });
  });
  ...
  $: if (deployer && controller$.valid) {
    deployer.get('nodeId').setValue(controller$.value);
    deployer.get('nodeId').markAsDirty();
  }
</script>
