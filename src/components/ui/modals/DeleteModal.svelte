  ...
  const validateDeletion = fb.control('', [
    ...
    (ctrl) => {
      if (ctrl.value != String(selectedInstances)) {
        return {
          message: `Instance name dose not match ${String(selectedInstances)}`,
        };
      }
    },
  ]);
...
<b-modal open>
  <b-modal-header>
    <b-btns align="centered" addons>
      You are about to delete your threebot deployer instance
      <strong
      ...
      {selectedInstances.length <= 1 ? 'instance' : 'instances'}, Are you
      absolutely sure?
    </b-btns>
  </b-modal-header>
  ...
  <b-modal-body>
    ...
    contains its all records posts, comments, users and all other tables. Please
    type
    <strong style:color="rgb(240 58 95)">
      {String(selectedInstances)}
    </strong>
    to confirm.
  ...
  <b-modal-actions>
    <b-btns align="right" size="small">
      ...
      >
        Delete {selectedInstances.length <= 1
          ? 'this instance'
          : 'those instances'}
      </button>
